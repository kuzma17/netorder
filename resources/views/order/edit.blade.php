                    ...
                    <div class="panel-body">
                        <form name="order_order" method="post" class="form-horizontal" action="{{ route('order.edit', ['id'=>$order->id]) }}" enctype="multipart/form-data">
                                ...
                                <div class="col-md-9">
                                    <select name="type_work" class="form-control">
                                        @foreach($order->typeWorks() as $type)
                                            <option value="{{ $type->id }}" @if($type->id == $order->typeWork->id) selected="selected" @endif>{{ $type->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group{{ $errors->has('date_end') ? ' has-error' : '' }}">
                                ...
                                <div class="col-md-9">
                                    <input class="form-control" type="date" name="date_end" value="{{old('date_end', $order->date_end)}}">
