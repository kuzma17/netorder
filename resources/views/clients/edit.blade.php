            ...
            <div class="col-md-8 col-md-offset-2">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <form name="order_order" method="post" class="form-horizontal" action="{{ route('client.edit', ['id'=>$client->id]) }}" >
                            {{ csrf_field() }}
                                ...
                                <div class="col-md-9">
                                    <select name="firm" class="form-control">
                                        @foreach($client->firms() as $firm)
                                            <option value="{{ $firm->id }}" @if($client->firm_id == $firm->id) selected @endif >{{ $firm->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-md-9">
                                    <select name="region" class="form-control">
                                        @foreach($client->regions() as $region)
                                            <option value="{{ $region->id }}" @if($client->region_id == $region->id) selected @endif>{{ $region->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-md-9">
                                    <select name="town" class="form-control">
                                        @foreach($client->towns() as $town)
                                            <option value="{{ $town->id }}" @if($client->town_id == $town->id) selected @endif>{{ $town->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-md-9">
                                    <select name="user" class="form-control">
                                        @foreach($client->users() as $user)
                                            <option value="{{ $user->id }}" @if($client->user_id == $user->id) selected @endif>{{ $user->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
